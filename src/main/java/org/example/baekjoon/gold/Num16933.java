...
public class Num16933 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        bfs(new Coordinate(0, 0, K, 1, true));
    }
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (map[ny][nx] == 0 && !visited[ny][nx][current.isBreak]) {
                    q.add(new Coordinate(nx, ny, current.isBreak, current.count + 1, !current.isMorning));
                    visited[ny][nx][current.isBreak] = true;
                }
                if (current.isBreak > 0) {
                    if (current.isMorning) {
                        if (map[ny][nx] == 1 && !visited[ny][nx][current.isBreak-1]) {
                            q.add(new Coordinate(nx, ny, current.isBreak-1, current.count + 1, false));
                            visited[ny][nx][current.isBreak-1] = true;
                        }
                    }else{
                        if (map[ny][nx] == 1 && !visited[ny][nx][current.isBreak-1]) {
                            q.add(new Coordinate(current.x, current.y, current.isBreak, current.count + 1, true));
                        }
                    }
                }
            }
    ...
    public static class Coordinate {
        ...
        public Coordinate(int x, int y, int isBreak, int count, boolean isMorning) {
            this.x = x;
            this.y = y;
